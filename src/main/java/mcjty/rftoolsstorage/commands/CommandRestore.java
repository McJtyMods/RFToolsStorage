    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (!(stack.getItem() instanceof StorageModuleItem)) {
            context.getSource().sendFeedback(
                    new StringTextComponent("Keep a storage module in your main hand!").applyTextStyle(style -> style.setColor(TextFormatting.RED)), true);
            return 0;
        }
        ...
        for (StorageEntry storage : holder.getStorages()) {
            if (storage.getUuid().toString().startsWith(uuidString)) {
                if (foundEntry != null) {
                    context.getSource().sendFeedback(
                            new StringTextComponent("Multiple storage entries match this UUID part!").applyTextStyle(style -> style.setColor(TextFormatting.RED)), true);
                    return 0;
                }
                foundEntry = storage;
            }
        ...
        if (foundEntry != null) {
            if (foundEntry.getStacks().size() != maxSize) {
                context.getSource().sendFeedback(
                        new StringTextComponent("Wrong foundEntry module tier! " + foundEntry.getStacks().size() + " stacks are required!").applyTextStyle(style -> style.setColor(TextFormatting.RED)), true);
            } else {
            ...
            context.getSource().sendFeedback(
                    new StringTextComponent("No storage found with UUID " + uuidString).applyTextStyle(style -> style.setColor(TextFormatting.RED)), true);
        }
        return 0;
    }
}
